...
class Glyph(object) :
    ...
    def add_anchor(self, anchor):
        self.anchors[anchor.name] = (int(anchor.x), int(anchor.y))
...
class Font(object) :
    def __init__(self, defines = None, psfCompatible = False):
    ...
    def readaps(self, f, omitaps='', ignoreglyphsRE=None):
        ...
        for g in f.keys():
            ...
            # get glyph bounding
            if self.psfCompatible:
                # Implement [faulty] pysilfont-compatible algorithm by looking only at contours
                if len(ufo_g):
                    bbox = (65536, 65536, -65536, -65536)
                    for c in ufo_g.contours:
                        for p in c.points:
                            if p.type:      # any on-curve point counts
                                x = float(p.x)
                                y = float(p.y)
                ...
                # Use the preferred ufoLib2 method
...
def main():
    ...
    parser.add_argument("--ignoreglyphs", const=r'^_', default=None, nargs='?', help="regEX describing glyphs to ignore")
